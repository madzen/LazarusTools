###############################################################################
# LazarusTools.py
# Toolkit for Vampire The Masquerade: Bloodlines
###############################################################################
# Copyright (c) 2009-2012 Madzen <dev@madzen.co.uk>
#
...
def lazFeed():
    ...
    if (victim.GetName() != "Caine"):
        try:
            pc.SeductiveFeed(victim)
        except:
            print "Unable to feed on victim"
...
def lazSparkle(item):
    ...
    #name = item.GetName()
    itemDist = lazDist(lazPlayerLoc, point)
    if (itemDist > 0):
        try:
